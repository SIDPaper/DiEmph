...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset_path", type=str, default='./small_test', help="Path to the dataset")
    ...
    # get basename of model path
    model_name = os.path.basename(args.model_path)
    # get basename of dataset path
    dataset_name = os.path.basename(args.dataset_path)
    # get basename of experiment path
    experiment_name = os.path.basename(args.experiment_path)
    logger = get_logger(f"jTrans-{model_name}-eval-{dataset_name}_savename_{experiment_name}_{tim}")
    ...
    opt =['O0', 'O3']
    # opt =['O0', 'O2', 'O3']
    ft_valid_dataset=FunctionDataset_CL(tokenizer,args.dataset_path,None,True,opt=opt, add_ebd=True, convert_jump_addr=True, ori=True)
