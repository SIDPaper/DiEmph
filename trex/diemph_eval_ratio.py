...
def parse_args():
    ...
    parser.add_argument('--model', type=str,
                        # default='checkpoints/jtrans-small-bf/checkpoint40.pt', help='checkpoint path')
                        default='checkpoints/jtrans-bf-ops/checkpoint_1_25000.pt', help='checkpoint path')
    parser.add_argument('--stack-model', type=str,
                        default='checkpoints/jtrans-bf-stack/checkpoint_1_15000.pt')
    parser.add_argument('--data', type=str,
                        default='data-bin/jtrans-bf-stack', help='data path')
                        # default='data-bin/binkit-bf', help='data path')
    parser.add_argument(
        '--sample', type=str, default='test-cases/libcurlh-500-500.instance.pkl', help='sample')
    parser.add_argument(
        '--bf-removal', action='store_true', help='use original tokens')
    parser.add_argument(
        '--neg-num', type=int, default=500, help='number of negative samples')
    parser.add_argument(
        '--emb-in', type=str, default='test-cases/libcurlh-500-500.bf15k.emb.pkl', help='emb input path')
        # '--emb-in', type=str, default='test-cases/coreutilsh-500-500.bf25k.emb.pkl', help='emb input path')
...
def how_should_skip(ops):
    ...
    if 'push' in ops[0] and ops[1] != None and 'r12' in ops[1]:
        return 1
    # # model4: push rbx
    # if 'push' in ops[0] and ops[1] != None and 'rbx' in ops[1]:
    #     return 1
    # # mov rbp, rsp
    # if ('mov' in ops[0]
    #     and ops[1] != None and 'rbp' in ops[1]
    ...
    # # mov r15, rax
    # if ('mov' in ops[0]
    #     and ops[1] != None and 'r15' in ops[1]
    #         and ops[2] != None and 'rax' in ops[2]):
    #     return 1
...
def main():
    ...
    if args.emb_in != '':
        ...
        model.eval()
        if not ori:
            stack_model = TrexModel.from_pretrained(
                os.path.dirname(args.stack_model), os.path.basename(args.stack_model),
                data_name_or_path=args.data)
        for func in tqdm(query_funcs):
            if ori:
                instr_list = get_instr_list(func, ori=ori, rewrite_strategy=args.rewrite_strategy)[:300]
            else:
                # instr_list = get_reordered_instr_list(func, ori=ori)[:300]
    ...
    np.random.seed(233)
    for sample in [16,32,50,100,200,300,500]:
        NEG_SAMPLE = sample        
            ...
            for func in tqdm(query_funcs):
                ...
                if args.baseline_emb != '':
                    ...
                    if (ori_len - new_len) / ori_len > 0.2 and len(func['cfg'].nodes) < 3:
                        ...
                        use_baseline = False
                    # use_baseline = len(get_instr_list(func, ori=True, rewrite_strategy='')) < 10
                    # use_baseline = len(func['cfg'].nodes) < 3
                ...
                if (ret > 0 and
                    ...
                    ret = 0.1
                results.append(ret)
            print("Ratio:%d, Round:%d"%(NEG_SAMPLE, sample_round))
            print("Top 1: %f=%d/%d" %
